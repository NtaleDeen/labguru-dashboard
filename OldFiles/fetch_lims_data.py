...
def parse_patient_table(html_content, search_method=""):
    """Parse the patient table from HTML content and return list of patient dicts."""
    patients = []
    try:
...
def fetch_patient_details(session, patient):
    """Fetch and parse test details for a given patient."""
    ...
    try:
        ...
        for row in rows[1:]:
            ...
            detail = {'TestName': cells[2].text.strip()}
...
def save_data(new_records):
    """Saves new records by appending to data.json."""
    if not new_records:
    ...
    # Convert records to a hashable format for comparison
