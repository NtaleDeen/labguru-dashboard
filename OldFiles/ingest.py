...
def get_client_id_by_identifier(cursor, client_identifier):
    """Get client ID from database using client identifier"""
    try:
        cursor.execute("SELECT id FROM clients WHERE identifier = %s", (client_identifier,))
        result = cursor.fetchone()
        return result[0] if result else None
    except Exception as e:
        logger.error(f"Error getting client ID for {client_identifier}: {e}")
        return None
...
def run_data_ingestion():
    ...
    try:
        ...
        client_identifier = os.getenv('CLIENT_IDENTIFIER', 'NHL_Lab')
        client_id = get_client_id_by_identifier(cursor, client_identifier)
        
        if not client_id:
            logger.error(f"Client identifier '{client_identifier}' not found in database. Please create client first.")
            return
        
        logger.info(f"Using client ID: {client_id} for identifier: {client_identifier}")
        ...
        new_tests_data = [
            (
                ...
                rec['Urgency'], parse_datetime_field(rec['Test_Time_Out']), client_id
            )
        ...
        if new_tests_data:
            test_columns = ['id', 'lab_number', 'test_name', 'lab_section', 'tat', 'price',
                            'time_received', 'test_time_expected', 'urgency', 'test_time_out', 'client_id']
        ...
        new_patients_data = [
            (
                rec['Lab_Number'], rec['Client'], client_id, rec['Date'], rec['Shift'], rec['Unit'],
        ...
        if new_patients_data:
            patient_columns = ['lab_number', 'client', 'client_id', 'date', 'shift', 'unit',
